...
export async function requestBot(nativeId: string): Promise<void> {
  const body: Record<string, unknown> = {
    ...
    automatic_leave: {
      ...
      // Vexa = 2ч и срезал длинные миты (Strat Sync 30.07) → подняли до 3ч по просьбе
      // Максима. Разово продлить идущий мит можно отменой стоп-джобы в /scheduler/jobs.
      max_bot_time: 3 * 60 * 60_000, // 3 часа
    },
  };

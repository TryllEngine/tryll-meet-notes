...
export async function requestBot(nativeId: string): Promise<void> {
  const body: Record<string, unknown> = {
    ...
    // Бот заходит за 5 мин до старта; ждём людей до +10 мин после старта =
    // 15 мин окна от момента захода. Как только зашёл хотя бы 1 человек —
    // Vexa отменяет ожидание и сразу начинает запись (ждать больше не нужно).
    automatic_leave: {
      max_wait_for_admission: 15 * 60_000, // 15 мин ждёт впуска (−5 до старта … +10 после)
      no_one_joined_timeout: 15 * 60_000, // 15 мин если так никто и не зашёл → выходит, заметок нет

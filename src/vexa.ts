...
export async function requestBot(nativeId: string): Promise<void> {
  const body: Record<string, unknown> = {
    ...
    automatic_leave: {
      // Бот заходит за 5 мин до старта. «Никто не зашёл» считаем так, чтобы он ждал
      // людей до +10 мин ПОСЛЕ старта по календарю = 15 мин от захода (5 до + 10 после).
      // Пример: мит в 14:00, бот зашёл 13:55, уйдёт в 14:10, если никто не пришёл.
      // (Vexa считает от захода бота, а заход = старт−5 мин, поэтому 15 мин.)
      max_wait_for_admission: 15 * 60_000,
      no_one_joined_timeout: 15 * 60_000, // 15 мин от захода ≈ 10 мин после старта
      max_time_left_alone: 3 * 60_000, // все вышли, бот один 3 мин → выходит и делает заметки
      // Абсолютный потолок жизни бота (server-enforced планировщиком Vexa). Дефолт
      // Vexa = 2ч и срезал длинные миты (Strat Sync 30.07) → подняли до 3ч, а после
      // 3-часового Strat Sync 03.09 — до 4ч, чтобы был запас на овертайм.
      // Разово продлить идущий мит можно отменой стоп-джобы в /scheduler/jobs.
      max_bot_time: 4 * 60 * 60_000, // 4 часа
    },
  };
